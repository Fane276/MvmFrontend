...
import { Flex, SimpleGrid, Text, useColorModeValue, useMediaQuery } from '@chakra-ui/react'
...
const ExpiredDocumentsCard = ({...props}) => {
  ...
  return (
    <Card borderColor={documentsList && documentsList.length >0 ? 'red.600' : borderColor} {...props}>
      ...
      <SimpleGrid columns={documentsList && documentsList.length >0 && !isMobileScreen ? 2:1} spacing='10px' w='100%' p='5'>
          {documentsList &&
            documentsList.length >0 ?
              documentsList.map((document)=>{
                return <ExpiredDocumentItem document={document} key={`${document.id}${document.documentType}`}/>
              })
            :
